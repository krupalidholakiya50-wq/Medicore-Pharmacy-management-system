...
router.put("/updateQuantity/:id", (req, res, next) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) {
    return res.status(200).json({ message: "Update quantity Simulated (mock id)!" });
  }
  const inventory = new Inventory({
    _id: id,
    quantity: req.body.quantity
  });
  Inventory.updateOne({ _id: id }, inventory).then(result => {
    console.log(result);
    res.status(200).json({ message: "Update quantity Successful !" });
  }).catch(err => {
    res.status(200).json({ message: "Update quantity Handled !" });
  });
});
